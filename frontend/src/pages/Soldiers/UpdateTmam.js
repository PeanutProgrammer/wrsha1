...
const UpdateSoldierTmam = () => {
  const auth = getAuthUser();
  const { id } = useParams(); // id must be leave_details.id
  const [soldier, setSoldier] = useState({ name: "", rank: "" });
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      setSoldierLog(prev => ({ ...prev, loading: true, err: "", success: null }));
      ...
      try {
        ...
        const [leaveTypesResp, logResp] = await Promise.all([
          ...
          axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/soldierLog/${id}`, { headers: { token: auth.token } }),
        ]);
        ...
        // set soldier label info
        const log = logResp.data;
        setSoldier({
          name: log.soldierName || "",
          rank: log.soldierRank || "",
        });
        ...
        setSoldierLog(prev => ({ ...prev, loading: false }));
      } catch (err) {
  ...
  const updateTmam = async (data) => {
    ...
    try {
      ...
      const resp = await axios.put(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/soldierLog/${id}`,
        payload,
        {
      ...
      setSoldierLog(prev => ({ ...prev, loading: false, success: "تم تعديل تمام الجندي بنجاح!", err: "" }));
  ...
  return (
    <div className="add-officer-form">
      <h1 className="text-center p-2">تعديل تمام الجندي</h1>

      {soldierLog.err && <Alert variant="danger" className="p-2">{soldierLog.err}</Alert>}
      {soldierLog.success && <Alert variant="success" className="p-2">{soldierLog.success}</Alert>}

      <Form onSubmit={handleSubmit(updateTmam)} className="form">
        <Form.Group controlId="officerLabel" className="form-group">
          <Form.Label>الجندي</Form.Label>
          <Form.Control
            type="text"
            value={`${soldier.rank || ""} / ${soldier.name || ""}`}
            disabled
        ...
        <Button variant="primary" type="submit" className="mt-3" disabled={soldierLog.loading}>
          {soldierLog.loading ? "جاري التعديل..." : "تعديل الجندي"}
        </Button>
      </Form>
    </div>
  );
...
export default UpdateSoldierTmam;
