...
const AddSoldierTmam = () => {
  ...
  const createOfficerLog = async (data) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/soldierLog/tmam`, formattedData, {
  ...
  // Transform soldiers into format required by react-select
  const soldierOptions = soldier.data?.map((soldier) => ({
    value: soldier.id,
    ...
    <div className="add-officer-form">
      ...
      <Form onSubmit={handleSubmit(createOfficerLog)} className="form">
        ...
        <Button type="submit" variant="primary" className='submit-btn' disabled={soldierLog.loading}>
          {soldierLog.loading ? 'جاري الإضافة...' : 'تسجيل التمام'}
        </Button>
      </Form>
    </div>
  );
