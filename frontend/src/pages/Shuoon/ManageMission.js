...
const ManageShuoonMission = () => {
  const auth = getAuthUser();
  const [sortConfig, setSortConfig] = useState({ key: "", direction: "asc" });
  const [filterFixedMission, setFilterFixedMission] = useState(false);
  ...
  const [units, setUnits] = useState({
    ...
    filterFixedMission: false, // Track fixed mission filter
  });
  ...
  const url = `${process.env.REACT_APP_BACKEND_BASE_URL}/shuoon/missions?page=${units.page}&limit=${units.limit}&search=${searchValue}&type=${units.selectedType}&filterFixedMission=${units.filterFixedMission}`;
  ...
  }, [units.page, units.search, units.selectedType, units.filterFixedMission]); // Dependency array includes filterFixedMission
  ...
  const filteredUnits = useMemo(() => {
    ...
    if (filterFixedMission) {
      result = result.filter(
        (unit) => unit.leave_type_name === "مأمورية ثابتة"
      );
    }
    // Apply sorting
    if (sortConfig.key) {
  ...
  return (
    <div className="Officers p-5">
      ...
      <div className="header d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
        <h3 className="text-white">إدارة مأموريات الصف والجنود</h3>
  ...
  <DropdownButton
    ...
    <Dropdown.Item eventKey="ncos">ضباط الصف</Dropdown.Item>
        ...
        {/* Filter Toggle */}
       {/* Filter Toggle for Fixed Mission */}
<Form.Check
  type="checkbox"
  label="عرض مأمورية ثابتة فقط"  // "Show only fixed mission"
  checked={units.filterFixedMission}
          onChange={() => setUnits((prev) => ({ ...prev, filterFixedMission: !prev.filterFixedMission }))}
                       ...
                       {filteredUnits.length > 0 ? (
                         filteredUnits.map((unit, index) => (
                           <tr key={unit.mil_id}>
                             ...
                             <td>{unit.leave_type_name === "مأمورية ثابتة" ? "مأمورية ثابتة" : "مأمورية"}</td>
...
export default ManageShuoonMission;
