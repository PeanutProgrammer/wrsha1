...
const ManageShuoonCourse = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      ...
      const resp = axios
        .get(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/shuoon/courses?page=${officers.page}&limit=${officers.limit}&search=${searchValue}`,
          {
  ...
  return (
    <div className="Officers p-5">
      ...
      <div className="header d-flex flex-wrap justify-content-between align-items-center mb-3 gap-2">
        {/* Page Title */}
        <h3 className="text-white">إدارة فرق / دورات ضباط الصف</h3>
            ...
            <tr>
              ...
              <th onClick={() => handleSort("department")}>
                {sortConfig.key === "department"
                  ...
                  : ""}
                الورشة / الفرع</th>
              <th onClick={() => handleSort("course_location")}>
                {sortConfig.key === "course_location"
                  ? sortConfig.direction === "asc"
                    ? " 🔼"
                    : " 🔽"
                  : ""}
                مكان الدورة / الفرقة</th>
            ...
            {filteredOfficers.length > 0 ? (
              filteredOfficers.map((officer, index) => (
                <tr key={officer.mil_id}>
                  ...
                  <td>{officer.destination || "لا يوجد"}</td>
                  <td>
                    {officer.start_date
...
export default ManageShuoonCourse;
