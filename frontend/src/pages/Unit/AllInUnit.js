...
const AllInUnit = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      axios
        .get(`${process.env.REACT_APP_BACKEND_BASE_URL}/unit/`, {
  ...
  const currentRecords = units.results.slice(indexOfFirstRecord, indexOfLastRecord);

  // Change page
  const paginate = (pageNumber) => setCurrentPage(pageNumber);

  // Calculate total pages
  const totalPages = Math.ceil(units.results.length / recordsPerPage);
  ...
  //               { text: 'الرتبة / الدرجة', style: 'tableHeader' },
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
          <thead>
            <tr style="background-color: #f2f2f2;">
              ...
              <th style="border: 1px solid black; padding: 5px;">الرتبة / الدرجة</th>
