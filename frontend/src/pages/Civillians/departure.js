...
const schema = yup.object().shape({
  ...
  civillianID: yup.number().required(' اسم المدني مطلوب '),
...
const CivillianDeparture = () => {
  ...
  const createOfficerLog = async (data) => {
    ...
    setCivillianLog({ ...civillianLog, loading: true });
    ...
    const formattedData = {
      ...
      end_date:  moment(data.end_date).locale("en").format("YYYY-MM-DD"),
    };

    console.log("Formatted Request Data:", formattedData);

    try {
      await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/civillianLog/departure`, formattedData, {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BACKEND_BASE_URL}/civillian/current`, {
        headers: {
          token: auth.token,
        },
      })
      .then((resp) => setCivillian(resp.data))
  ...
  return (
    <div className="add-officer-form">
      ...
      <Form onSubmit={handleSubmit(createOfficerLog)} className="form">
        ...
        <Form.Group controlId="civillianID" className="form-group">
          <Form.Label>الاسم</Form.Label>
          <Select
            {...register("civillianID")} // Bind the civillianID value from react-hook-form
            options={civillianOptions}
            getOptionLabel={(e) => e.label}
            getOptionValue={(e) => e.value}
            onChange={handleCivillianChange} // Update civillianID on selection
        ...
        >
          {civillianLog.loading ? "جاري الإضافة..." : "تسجيل خروج"}
        </Button>
      </Form>
    </div>
  );
...
export default CivillianDeparture;
