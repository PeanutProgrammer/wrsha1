...
const AddNCOTmam = () => {
  ...
  // Transform ncos into format required by react-select
  const ncoOptions = nco.data?.map((nco) => ({
    value: nco.id,
    label: `${nco.rank} / ${nco.name}`,
  }));

  // Handle when an nco is selected
  const handleNCOChange = (selectedOption) => {
    if (selectedOption) {
