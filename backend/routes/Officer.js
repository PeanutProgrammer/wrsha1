...
router.post(
  ...
  body("name")
    ...
    .withMessage("اسم الضابط يجب أن يكون أكثر من 3 حروف ولا يتجاوز 50 حرفًا"),
  body("join_date")
    .isDate()
  ...
  body("department")
    .isString()
    .withMessage("يرجى إدخال الفرع أو الورشة بشكل صحيح"),
  body("height").isNumeric().withMessage("يرجى إدخال طول صحيح"),
  body("weight").isNumeric().withMessage("يرجى إدخال وزن صحيح"),
  body("dob").isDate().withMessage("يرجى إدخال تاريخ ميلاد صحيح"),
  ...
  body("seniority_number")
    .isString()
    .withMessage("يرجى إدخال رقم الأقدمية بشكل صحيح"),
  body("attached")
    .optional()
    .isBoolean()
...
router.get(
  "/daily-summary",
  allowAny(security, leader, shuoonOfficers),
  (req, res) => {
...
router.get("/:id", allowAny(shuoonOfficers,leader), (req, res) => {
