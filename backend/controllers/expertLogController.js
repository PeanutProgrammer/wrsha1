...
class ExpertLogController {
  ...
  static async endVisit(req, res) {
    try {
      ...
      await withTransaction(async (query) => {
        ...
        await query("UPDATE expert_record SET end_date = ? WHERE id = ?", [
          visit_end,
          req.params.id,
        ]);

        await query("update experts set in_unit = 0 where id = ?", [
  ...
  static async createArrival(req, res) {
    try {
      ...
      await withTransaction(async (query) => {
        ...
        await query(
          "insert into expert_record set start_date = ?, end_date = ?, expertID = ?, officerID = ?, external_officer = ?,loggerID = ?, notes = ?",
          [
            expertObject.getStartDate(),
            expertObject.getEndDate(),
            expertObject.getExpertID(),
            expertObject.getOfficerID(),
            expertObject.getExternalOfficer(),
            expertObject.getLoggerID(),
            expertObject.getNotes(),
          ]
        );

        await query("update experts set in_unit = 1 where id = ?", [
          expertObject.getExpertID(),
        ]);
      });
      req.app.get("io").emit("expertsUpdated");
  ...
  static async createDeparture(req, res) {
    try {
      ...
      await withTransaction(async (query) => {
        ...
        const expertRecordResult = await query(
          "insert into expert_record set start_date = ?, end_date = ?, expertID = ?, officerID = ?, external_officer = ?, loggerID = ?, notes = ?",
          [
            expertObject.getStartDate(),
            expertObject.getEndDate(),
  ...
  static async getMonthlyExpertLog(req, res) {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
      ...
      const year = parseInt(req.query.year); // e.g., 2026
      ...
      // --- Total count for pagination ---
      const countResult = await query(`SELECT COUNT(*) AS total FROM experts`);
      const total = countResult[0].total;
      const totalPages = Math.ceil(total / limit);

      // Fetch paginated experts
      const experts = await query(
        `SELECT id, nationalID, name 
       FROM experts
       ORDER BY name
       LIMIT ? OFFSET ?`,
        [limit, offset]
      ...
      // Fetch all visits in that month for only the paginated experts
      const expertIds = experts.map((exp) => exp.id);
      const records = await query(
        `SELECT expertID, start_date, end_date
       FROM expert_record
       WHERE expertID IN (?) 
         AND start_date BETWEEN ? AND ?`,
        [expertIds, startDate, endDate]
      );

      // Map expertID -> array of days visited
      const expertVisits = {};
      ...
      records.forEach((rec) => {
        const start = moment(rec.start_date);
        const end = rec.end_date ? moment(rec.end_date) : start;
        const current = moment(start);
        while (
          current.isSameOrBefore(end) &&
          current.isSameOrBefore(moment(endDate))
        ) {
          expertVisits[rec.expertID].push(current.date());
          current.add(1, "day");
        }
      });

      // Combine data
      const result = experts.map((exp, idx) => ({
        م: (page - 1) * limit + idx + 1,
        id: exp.nationalID,
        name: exp.name,
        days: expertVisits[exp.id],
      }));
      ...
      console.error(err);
      return res.status(500).json({
        message: "Error fetching monthly expert log",
        error: err.message,
      });
    }
  }
