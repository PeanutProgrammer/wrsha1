...
class SoldierLogController {
    ...
    static async getSoldiersLog(req, res) {
        try {
            ...
            // }
            const soldiers = await query(`SELECT soldiers.mil_id, soldiers.rank, soldiers.name, soldiers.department, officer_log.event_type, officer_log.event_time, leave_type.name AS reason
                                          FROM soldiers
                                          LEFT JOIN soldier_log
                                          ON soldier_log.officerID = soldiers.id
