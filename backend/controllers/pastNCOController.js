...
class PastNCOController {
    ...
    static async getNCO(req, res) {
                try {
                    ...
                    const officer = await query("select * from past_ncos where id = ?",[req.params.id])
                    ...
                    const officerObject = new PastNCO(officer[0].name, officer[0].join_date,  officer[0].mil_id, officer[0].rank, officer[0].address, officer[0].height, officer[0].weight, officer[0].dob, officer[0].end_date, officer[0].transferID, officer[0].transferred_to);
