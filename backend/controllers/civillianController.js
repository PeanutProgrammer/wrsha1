...
class CivillianController {
    ...
    static async deleteCivillian(req, res) {
        try {
            ...
            await query("delete from civillians where nationalID = ?", [checkCivillian[0].nationalID])
    ...
    static async getCivilliansTmam(req, res) {
        try {
            ...
            const civillians = await query(`SELECT civillians.nationalID,civillians.name, civillians.department, leave_type.name AS 'tmam'
                                          FROM civillians
     ...
     static async getCivillianTmamDetails(req, res) {
        try {
            ...
            //     search =  `where name LIKE '%${req.query.search}%'`
            // }
            const civillian = await query("select * from civillians where nationalID = ?",[req.params.id])
            console.log("hey");
            if (civillian.length == 0) {
                return res.status(404).json({
                    msg: "no civillians found blah"
  ...
  try {
    ...
    const validCivillians = civillians.filter(
      (civillian) => moment(civillian.join_date).isBefore(now)
    );
